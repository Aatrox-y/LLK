...
BEGIN_MESSAGE_MAP(CGameDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_LBUTTONUP()
	ON_BN_CLICKED(IDC_BTN_START, &CGameDlg::OnBnClickedButtonStart)
	ON_BN_CLICKED(IDC_BTN_TIP, &CGameDlg::OnBnClickedButton3)
	ON_BN_CLICKED(IDC_BTN_AGAIN, &CGameDlg::OnBnClickedButton4)
	ON_WM_TIMER()
...
void CGameDlg::InitElement()
{
	...
	HBITMAP hPause = (HBITMAP)::LoadImage(NULL,_T("theme\\picture\\fruit_pause.bmp"),IMAGE_BITMAP,0, 0,LR_LOADFROMFILE | LR_CREATEDIBSECTION);
	m_bmpPause.Attach(hPause);
	m_dcPause.CreateCompatibleDC(&dc);
	m_dcPause.SelectObject(&m_bmpPause);

	// 初始化缓存DC
	CClientDC dcCache(this);
	m_dcCache.CreateCompatibleDC(&dcCache);
	...
	// 创建兼容位图
	m_bmpCache.CreateCompatibleBitmap(&dc, rect.Width(), rect.Height());
...
void CGameDlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	//第二个点
		...
		//判断胜负
		if (m_gameControl.IsWin(m_nRemainingTime)==0) {
...
void CGameDlg::OnBnClickedButtonStart()
{
	...
	m_nRemainingTime = 300;
	m_Progress.SetRange(0, 60 * 5);
	m_Progress.SetStep(-1);
	m_Progress.SetPos(60 * 5);
...
void CGameDlg::UpdateWindow()
{
	...
	//设置窗口大小
	MoveWindow(0, 0, 820 + nSpanWidth, 660 + nSpanHeight);
	CenterWindow();
}
void CGameDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (nIDEvent == 1)  // 判断是否是我们的定时器
	{
		m_nRemainingTime--;  // 剩余时间减1秒

		// 更新进度条（从30递减到0）
		m_Progress.SetPos(m_nRemainingTime);

		// 如果倒计时结束
		if (m_nRemainingTime <= 0)
		{
			KillTimer(1);  // 停止定时器
			MessageBox(_T("时间到！"), _T("提示"), MB_OK);
			m_bPlaying = false;
			this->GetDlgItem(IDC_BTN_START)->EnableWindow(true);
		}
	}
...
void CGameDlg::SaveGameMap()
{
	...
	// 将游戏地图保存到缓存DC
	m_dcCache.BitBlt(0, 0, rect.Width(), rect.Height(), &dc, 0, 0, SRCCOPY);
}
void CGameDlg::OnClickedBtnStop()
{
	if (!m_bPlaying && !m_bPause) return;
	...
	if (m_bPause)
	{
		...
		// 获取暂停图片的尺寸
		BITMAP bmpInfo;
		m_bmpPause.GetBitmap(&bmpInfo);

		// 绘制暂停图片（透明背景）
		dc.BitBlt(30, 30, 660,660, &m_dcPause, 0, 0, SRCCOPY);

		pBtn->SetWindowText(_T("继续游戏"));
	...
	else
	{
		...
		CRect rect;
		GetClientRect(&rect);
		dc.BitBlt(0, 0, rect.Width(), rect.Height(), &m_dcCache, 0, 0, SRCCOPY);

		// 修改按钮文本
		pBtn->SetWindowText(_T("暂停游戏"));
